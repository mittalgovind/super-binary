...
split_funcs = dict()
jump_targets_all = []
jump_targets_all_raw = dict()
for func_name, instructions in sorted(functions.iteritems()):
	split_funcs[func_name] = dict()
	jump_targets = []
	jump_targets_raw = []
	...
	for instr in instructions:
		...
		if jump_match:
			...
			jump_targets_raw += [(jump_match.group(2), jump_no_in_func)]
			jump_no_in_func += 1
			flag = 1
			continue
		if flag == 1:
	...
	for instr in instructions:
		...
		if jump_targets.__contains__(addr_match.group(1)):
			...
			bbs[curr_block] += [instr]
	split_funcs[func_name] = bbs
	jump_targets_all += [sorted(set(jump_targets))]
	jump_targets_all_raw[func_name] = jump_targets_raw

G = dict()
graph_no = 0
for func_name, bbs in sorted(split_funcs.iteritems()):
